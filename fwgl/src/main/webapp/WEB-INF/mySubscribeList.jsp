...
<div class="zy_right_con">
                    ...
                    <tr>
                        ...
                        <td class="todo-list">
                            ...
                            <span class="iconfont icon-shenhe" style=" color: #1945dd;cursor:pointer;font-weight: bold;font-size: 15px;" title="查看审核结果" onclick="queryAudit('${subscribeList[status.index].id}')"></span>
                                <c:choose>
                                    <c:when test="${subscribeList[status.index].status eq 1}">
                                        <%--<span class="iconfont icon-zuofei" style="font-weight: bold;font-size: 15px;" title="取消申请" ></span>--%>
                                    </c:when>
                                    <c:when test="${subscribeList[status.index].status eq 2}">
                                        <span class="iconfont icon-quxiao" onclick="cancelSubscribe('${serviceInfo.id}')" style="color:red;font-weight: bold;font-size: 15px;" title="取消订阅"  ></span>
                                    </c:when>
                                    <c:when test="${subscribeList[status.index].status eq 3}">
                                        <span class="iconfont icon-dingyue" style=" font-weight: bold;font-size: 15px;" title="再次订阅" onclick="subscribe('${serviceInfo.id}')"></span>
                                    </c:when>
                                    <c:when test="${subscribeList[status.index].status eq 4}">
                                        <%--<span class="iconfont icon-zuofei" style="font-weight: bold;font-size: 15px;" title="取消申请" ></span>--%>
                                    </c:when>
                                    ...
                                    <c:otherwise>
                                        <span class="iconfont icon-quxiao" style="color:red;font-weight: bold;font-size: 15px;" title="取消订阅" onclick="cancelSubscribe('${serviceInfo.id}')" ></span>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                        </td>
    ...
    <script>
        ...
        function homePage(){
            ...
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo=1&findType="+findType+"&pageSize="+$("#pageSize").val();
                ...
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo=1&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                ...
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo=1&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
        function previousPage(pageNo){
            var findType=$("#findType").val();
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo="+pageNo+"&findType="+findType+"&pageSize="+$("#pageSize").val();
            }else if(findType=="1"||findType=="2"){
                var searchName=$("#searchName").val();
                if(searchName==''){
                    if(findType=="1") {
                ...
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo="+pageNo+"&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                var logDate=$("#logDate").val();
                if(logDate==''){
                    $.alert({
                ...
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?pageNo="+pageNo+"&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
            ...
            if(findType==0){
                ...
                if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                    ...
                    if(findType==1){//根据名称查询
                        window.location.href="${pageContext.request.contextPath}/mySubscribeList?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 2){
                        window.location.href="${pageContext.request.contextPath}/mySubscribeList?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 3){
                        ...
                        if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                            ...
                            window.location.href="${pageContext.request.contextPath}/mySubscribeList?range="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                        }
                    }
                }
            }
        ...
        function downServiceFile() {
            ...
            $("#pageSize").change(function(){
                ...
                window.location.href="${pageContext.request.contextPath}/mySubscribeList?findType=0&pageNo=1&pageSize="+pageSize;
            });
