            ...
            <table class="table-striped">
                ...
                <c:forEach items="${serviceInfoList}" var="serviceInfo" varStatus="status">
                    <tr>
                        ...
                        <td> <span class="label label-info">申请取消订阅</span></td>
                        <td class="todo-list">
                            <div class="tools">
                            <%--<span class="iconfont icon-chakan"   title="查询接口详情" onclick="showServiceInfo('${serviceInfo.id}');"></span>--%>
                            <span class="iconfont icon-shenhejieguo"   title="取消订阅审核" onclick="cancelAudit('${subscribeList[status.index].id}')"></span>
                            </div>
                        </td>
                    </tr>
                </c:forEach>
    ...
    <div class="modal-dialog" role="document" style="width:350px;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel2">服务取消订阅申请审核</h4>
            </div>
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea name="reason" id="reason" cols="15" rows="3" class="form-control" placeholder="请填写拒绝理由" disabled="true"></textarea>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" id="btn_audit" onclick="cancelAuditApply()" class="btn btn-primary"><span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span>保存</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade bs-example-modal-lg" id="unloadModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script>
        ...
        function cancelAuditApply() {
            var Sid = $("#Sid").val();
            var reason = $("#reason").val();
            var result = $("input[name='radio']:checked").val();
            $.ajax({
                url : "${pageContext.request.contextPath}/sub/cancelSubAudit",
                async : true,
                type : "POST",
                dataType : "json",
                data : {
        ...
        function homePage(){
            ...
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo=1&findType="+findType+"&pageSize="+$("#pageSize").val();
                ...
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo=1&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                ...
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo=1&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
        function previousPage(pageNo){
            var findType=$("#findType").val();
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo="+pageNo+"&findType="+findType+"&pageSize="+$("#pageSize").val();
            }else if(findType=="1"||findType=="2"){
                var searchName=$("#searchName").val();
                if(searchName==''){
                    if(findType=="1") {
                ...
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo="+pageNo+"&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                var logDate=$("#logDate").val();
                if(logDate==''){
                    $.alert({
                ...
                window.location.href="${pageContext.request.contextPath}/unSubscribeList?pageNo="+pageNo+"&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
            ...
            if(findType==0){
                ...
                if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                    ...
                    if(findType==1){//根据名称查询
                        window.location.href="${pageContext.request.contextPath}/unSubscribeList?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 2){
                        window.location.href="${pageContext.request.contextPath}/unSubscribeList?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 3){
                        ...
                        if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                            ...
                            window.location.href="${pageContext.request.contextPath}/unSubscribeList?range="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                        }
                    }
                }
            }
        ...
        $("#pageSize").change(function(){
            ...
            if(findType==0){
                ...
                if(findType==1){//根据名称查询
                    let searchName = $("#searchName").val();
                    window.location.href="${pageContext.request.contextPath}/?categoryList="+searchName+"&findType=1&pageNo=1"+"&pageSize="+pageSize;
                }else if(findType == 2){
                    let searchName = $("#searchName").val();
                    window.location.href="${pageContext.request.contextPath}/categoryList?name="+searchName+"&findType=2&pageNo=1"+"&pageSize="+pageSize;
                }else if(findType == 3){
                    let time = $("#logDate").val();
                    window.location.href="${pageContext.request.contextPath}/categoryList?range="+time+"&findType=3&pageNo=1&pageSize="+pageSize;
                }
            }
        });
