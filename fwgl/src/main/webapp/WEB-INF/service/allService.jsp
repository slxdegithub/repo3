...
<div class="zy_right_con">
    ...
    <div style="width: 100%;position: relative;">
        ...
        <div class="search_div">
            ...
            <input type="text" class="search-input" id="searchName" placeholder="" value="${name}" autocomplete="off" />
            <input type="text" class="search-input" id="logDate" placeholder="请选择时间范围" value="${time}" autocomplete="off">
        ...
        <table class="table-striped">
            ...
            <c:forEach items="${serviceInfoList}" var="serviceInfo" varStatus="status">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${serviceInfo.status eq 0}">
                                <span class="label label-default">待挂载</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 1}">
                                <span class="label label-info">挂载审核</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 2}">
                                <span class="label label-success">已挂载</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 3}">
                                <span class="label label-warning">挂载拒绝</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 4}">
                                <span class="label label-info">作废审核</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 5}">
                                <span class="label label-success">已作废</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 6}">
                                <span class="label label-warning">作废拒绝</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 7}">
                                <span class="label label-info">卸载审核</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 8}">
                                <span class="label label-success">已卸载</span>
                            </c:when>
                            <c:otherwise>
                                <span class="label label-warning">卸载拒绝</span>
                            </c:otherwise>
                        </c:choose>
                    </td>
                    <td class="todo-list">
                        <div class="tools " >
                            <span class="iconfont icon-rizhi thin_icon"  title="操作记录"   onclick="flowInfo('${serviceInfo.id}')"></span>
                        </div>
                    </td>
                </tr>
            </c:forEach>
    ...
    <div class="modal-dialog" role="document" style="width:800px;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="infoId">审核流程</h4>
            </div>
...
<script>
    ...
    function goNext(pageNo){
        ...
        if(findType == 0){
            window.location.href="${pageContext.request.contextPath}/allService?pageNo="+pageNo+"&findType=0&pageSize="+$("#pageSize").val();
        }else if(findType == 1){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/allService?name="+searchName+"&findType=1&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 2){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/allService?name="+searchName+"&findType=2&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 3){
            var time = $("#logDate").val();
            window.location.href="${pageContext.request.contextPath}/allService?time="+time+"&findType=3&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }
    }
    ...
    function flowInfo(id) {
        $("#infoId").val(id);
        $.ajax({
            url : "${pageContext.request.contextPath}/serviceAudit/flowInfo",
            async : true,
            type : "POST",
            dataType : "text",
            data : {
    ...
    function showUnloadModel(data) {
        ...
        var auditUtils = data.split(";;;");
        var i = 0;
        for(;i<auditUtils.length;i++){
            console.log("auditUtil1="+auditUtils[i]);
            var auditUtil =  jQuery.parseJSON(auditUtils[i]);
            console.log("auditUtil2="+auditUtil);
            var tr = "<tr class='auditInfos'>";
            var td1 = "<td class=\"text-center\">"+(i+1)+"</td>" ;
            var td2 = "<td class=\"text-center\">"+auditUtil.operate+"</td>";
            var td3 = "<td class=\"text-center\">"+auditUtil.result+"</td>" ;
            var td4 = "<td class=\"text-center\">"+auditUtil.auditTime+"</td>";
            var td5 = "<td class=\"text-center\">"+auditUtil.reason+"</td>" ;
            var tr2 = "</tr>";
            $("#unloadInfo").append(tr+td1+td2+td3+td4+td5+tr2);
        }
        ...
        if(findType==0){
            ...
            if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                ...
                if(findType==1){//根据名称查询
                    window.location.href="${pageContext.request.contextPath}/allService?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 2){
                    window.location.href="${pageContext.request.contextPath}/allService?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 3){
                    ...
                    if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                        ...
                        window.location.href="${pageContext.request.contextPath}/allService?time="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                    }
                }
            }
        }
    ...
    $("#pageSize").change(function(){
        ...
        window.location.href="${pageContext.request.contextPath}/allService?findType=0&pageNo=1&pageSize="+pageSize;
    });
</script>
</body>
</html>