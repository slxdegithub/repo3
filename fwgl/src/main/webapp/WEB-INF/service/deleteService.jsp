        ...
        <table class="table-striped">
            ...
            <c:forEach items="${serviceInfoList}" var="serviceInfo" varStatus="status">
                <tr>
                    ...
                    <td>${serviceInfo.unmountTime}</td>
                    <td>
                        <c:choose>
                            <c:when test="${serviceInfo.status eq 4}">
                                <span class="label label-default">作废审核</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 6}">
                                <span class="label label-warning">拒绝作废</span>
                            </c:when>
                            <c:otherwise>
                    ...
                    <td class="todo-list">
                        <div class="tools">
                        <c:if test="${(serviceInfo.status eq 8 or serviceInfo.status eq 6) and loginUser.role eq 2}">
                            <span class="iconfont icon-zuofei" title="申请作废" onclick="submitApply('${serviceInfo.id}')"></span>
                            <%--  申请挂载  --%>
                            <span class="iconfont icon-guazai" title="申请挂载" onclick="submitToApply('${serviceInfo.id}')"></span>
                        </c:if>
                        <c:if test="${serviceInfo.status eq 4 and loginUser.role eq 3}">
                            <span class="iconfont icon-shenhejieguo" title="作废申请审核" onclick="audit('${serviceInfo.id}')"></span>
                        </c:if>
                        <c:if test="${serviceInfo.status eq 6}">
                            <span class="iconfont icon-xiangqing" title="拒绝原因" onclick="unloadInfo('${serviceInfo.id}')"></span>
                        </c:if>
                        </div>
                    </td>
                </tr>
    ...
    <div class="modal-dialog" role="document" style="width:350px;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel2">作废申请审核</h4>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="btn_audit" class="btn btn-primary" onclick="auditApply();"><span class="glyphicon glyphicon-floppy-disk" aria-hidden="true"></span>保存</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog" role="document" style="width:400px;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="infoId">拒绝原因</h4>
            </div>
...
<script>
    ...
    function goNext(pageNo){
        ...
        if(findType == 0){
            window.location.href="${pageContext.request.contextPath}/deleteService?pageNo="+pageNo+"&findType=0&pageSize="+$("#pageSize").val();
        }else if(findType == 1){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/deleteService?name="+searchName+"&findType=1&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 2){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/deleteService?name="+searchName+"&findType=2&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 3){
            var time = $("#logDate").val();
            window.location.href="${pageContext.request.contextPath}/deleteService?time="+time+"&findType=3&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }
    }
    ...
    function unloadInfo(id) {
        $("#infoId").val(id);
        $.ajax({
            url : "${pageContext.request.contextPath}/serviceAudit/deleteInfo",
            async : true,
            type : "POST",
            dataType : "text",
            data : {
    ...
    /***********  8-14 查询  findType=1 :名称模糊查询  findType=2 类别查询**********/
        ...
        if(findType==0){
            ...
            if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                ...
                if(findType==1){//根据名称查询
                    window.location.href="${pageContext.request.contextPath}/deleteService?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 2){
                    window.location.href="${pageContext.request.contextPath}/deleteService?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 3){
                    ...
                    if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                        ...
                        window.location.href="${pageContext.request.contextPath}/deleteService?time="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                    }
                }
            }
        }
    ...
    $("#pageSize").change(function(){
        ...
        window.location.href="${pageContext.request.contextPath}/deleteService?findType=0&pageNo=1&pageSize="+pageSize;
    });
</script>
