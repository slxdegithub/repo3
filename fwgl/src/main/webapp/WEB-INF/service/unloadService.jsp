        ...
        <table class="table-striped">
            ...
            <c:forEach items="${serviceInfoList}" var="serviceInfo" varStatus="status">
                <tr>
                    ...
                    <td>${serviceInfo.mountTime}</td>
                    <td>
                        <c:choose>
                            <c:when test="${serviceInfo.status eq 7}">
                                <span class="label label-info">卸载审核</span>
                            </c:when>
                            <c:when test="${serviceInfo.status eq 9}">
                                <span class="label label-warning">拒绝卸载</span>
                            </c:when>
                            <c:otherwise>
                    ...
                    <td class="todo-list">
                        <div class="tools">
                        <c:if test="${(serviceInfo.status eq 2 or serviceInfo.status eq 9) and loginUser.role eq 2}">
                            <span class="iconfont icon-yunxiazai thin_icon"  title="申请卸载" onclick="submitApply('${serviceInfo.id}')"></span>
                        </c:if>
                        <c:if test="${serviceInfo.status eq 7 and loginUser.role eq 3}">
                            <span class="iconfont icon-shenhejieguo" title="卸载审核" onclick="audit('${serviceInfo.id}')"></span>
    ...
    <div class="modal-dialog" role="document" style="width:350px;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel2">服务卸载申请审核</h4>
            </div>
...
<script>
    ...
    function goNext(pageNo){
        ...
        if(findType == 0){
            window.location.href="${pageContext.request.contextPath}/unloadService?pageNo="+pageNo+"&findType=0&pageSize="+$("#pageSize").val();
        }else if(findType == 1){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/unloadService?name="+searchName+"&findType=1&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 2){
            let searchName = $("#searchName").val();
            window.location.href="${pageContext.request.contextPath}/unloadService?name="+searchName+"&findType=2&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }else if(findType == 3){
            let time = $("#logDate").val();
            window.location.href="${pageContext.request.contextPath}/unloadService?time="+time+"&findType=3&pageNo="+pageNo+"&pageSize="+$("#pageSize").val();
        }
    }
    ...
    function unloadInfo(id) {
        $("#infoId").val(id);
        $.ajax({
            url : "${pageContext.request.contextPath}/serviceAudit/unloadInfo",
            async : true,
            type : "POST",
            dataType : "text",
            data : {
        ...
        if(findType==0){
            ...
            if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                ...
                if(findType==1){//根据名称查询
                    window.location.href="${pageContext.request.contextPath}/unloadService?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 2){
                    window.location.href="${pageContext.request.contextPath}/unloadService?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                }else if(findType == 3){
                    let time = $("#logDate").val();
                    if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                        ...
                        window.location.href="${pageContext.request.contextPath}/unloadService?time="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                    }
                }
            }
        }
    ...
    $("#pageSize").change(function(){
        ...
        window.location.href="${pageContext.request.contextPath}/unloadService?findType=0&pageNo=1&pageSize="+pageSize;
    });
</script>
