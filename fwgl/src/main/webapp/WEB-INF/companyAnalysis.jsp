            ...
            <table class="table-striped">
                ...
                <c:forEach items="${serviceInfoList}" var="serviceInfo" varStatus="status">
                    <tr>
                        ...
                        <td>${subscribeList[status.index].describeTime}</td>
                        <td>
                            <c:choose>
                                <c:when test="${subscribeList[status.index].status eq 1}">
                                    <span class="label label-info"> 申请订阅</span>
                                </c:when>
                                <c:when test="${subscribeList[status.index].status eq 2}">
                                    <span class="label label-success">订阅成功</span>
                                </c:when>
                                <c:when test="${subscribeList[status.index].status eq 3}">
                                    <span class="label label-warning">订阅失败</span>
                                </c:when>
                                <c:when test="${subscribeList[status.index].status eq 4}">
                                    <span class="label label-info">申请取消订阅</span>
                                </c:when>
                                <c:when test="${subscribeList[status.index].status eq 5}">
                                    <span class="label label-success">取消订阅成功</span>
                                </c:when>
                                <c:otherwise>
                                    <span class="label label-warning">取消订阅失败</span>
                                </c:otherwise>
                            </c:choose>
                        </td>
                        <td>${times[status.index]}</td>
                        <td class="todo-list">
                            <div class="tools">
                                <span class="iconfont icon-chakan" title="查看详情" onclick="queryDetails('${serviceInfo.address}') "></span>
                            </div>
                        </td>
                    </tr>
                </c:forEach>
...
<div class="modal fade bs-example-modal-lg" id="accessInfoModel" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document" style="width:400px;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="accessLabel">服务调用详情</h4>
            </div>
            <div class="modal-body box">
                <div align="center">
                    <table id="accessInfo" class="table  dataTable">
                        <tr>
                            <th class="text-center">序号</th>
                            <th class="text-center">订阅者</th>
            ...
            <div class="modal-footer">
                <button type="button" id="closeUn1"  class="btn btn-default" data-dismiss="modal"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span>关闭</button>
            </div>
        </div>
    </div>
</div>
    <script>
        ...
        function homePage(){
            ...
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo=1&findType="+findType+"&pageSize="+$("#pageSize").val();
                ...
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo=1&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                ...
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo=1&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
        function previousPage(pageNo){
            var findType=$("#findType").val();
            if(findType=="0"){
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo="+pageNo+"&findType="+findType+"&pageSize="+$("#pageSize").val();
            }else if(findType=="1"||findType=="2"){
                var searchName=$("#searchName").val();
                if(searchName==''){
                    if(findType=="1") {
                ...
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo="+pageNo+"&findType="+findType+"&name="+searchName+"&pageSize="+$("#pageSize").val();
            }else{
                var logDate=$("#logDate").val();
                if(logDate==''){
                    $.alert({
                ...
                window.location.href="${pageContext.request.contextPath}/companyAnalysis?pageNo="+pageNo+"&findType="+findType+"&range="+logDate+"&pageSize="+$("#pageSize").val();
            }
        }
            ...
            if(findType==0){
                ...
                if(searchName.replace(/(^\s*)|(\s*$)/g, "").length ==0&&findType!=3){
                    ...
                    if(findType==1){//根据名称查询
                        window.location.href="${pageContext.request.contextPath}/companyAnalysis?name="+searchName+"&findType=1&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 2){
                        window.location.href="${pageContext.request.contextPath}/companyAnalysis?name="+searchName+"&findType=2&pageNo=1&pageSize="+$("#pageSize").val();
                    }else if(findType == 3){
                        ...
                        if(time.replace(/(^\s*)|(\s*$)/g, "").length ==0){
                            ...
                            window.location.href="${pageContext.request.contextPath}/companyAnalysis?range="+time+"&findType=3&pageNo=1&pageSize="+$("#pageSize").val();
                        }
                    }
                }
            }
        ...
        function queryDetails(address){
            $.ajax({
                url : "${pageContext.request.contextPath}/service/getAccessNumByAddress",
                async : true,
                type : "POST",
                dataType : "json",
                data : {
        ...
        function showAccess(data){
               // var reqtab = document.getElementById("accessInfo") ;
                $("#accessInfo tr:not(:first)").remove();
                for(i=0;i<data.userNameList.length;i++){
                    var tmp="<tr role='row' class='even' align='center'><td>"+(i+1).toString()+"</td>"+"<td>"+data.userNameList[i].toString()+"</td>"+
                        "<td>"+data.times[i].toString()+"</td></tr>"
                     $('#accessInfo').append(tmp);
                }
            $('#accessInfoModel').modal('show');
        }
        ...
        $("#pageSize").change(function(){
            ...
            window.location.href="${pageContext.request.contextPath}/companyAnalysis?findType=0&pageNo=1&pageSize="+pageSize;
        });
    </script>
